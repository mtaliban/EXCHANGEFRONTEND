...
export default function ServicesPage() {
  const t = useT();
  const services = [
    { icon: '🏥', title: t('svc.health'), desc: t('svc.health_d') },
    { icon: '👩‍🏫', title: t('svc.teachers'), desc: t('svc.teachers_d') },
    { icon: '🔎', title: t('svc.matching'), desc: t('svc.matching_d') },
    { icon: '💬', title: t('svc.chat'), desc: t('svc.chat_d') },
    { icon: '🔔', title: t('svc.notif'), desc: t('svc.notif_d') },
    { icon: '🛡️', title: t('svc.verify'), desc: t('svc.verify_d') },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map((s) => (
          <div key={s.title} className="card hover:shadow-lg transition group">
            <div className="w-14 h-14 rounded-xl bg-brand-blue-50 flex items-center justify-center text-3xl mb-4 group-hover:scale-110 transition">
              {s.icon}
            </div>
